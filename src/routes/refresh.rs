            ...
            tx_ins.execute(
                "UPDATE repos
                 SET ci_status = COALESCE(?1, ci_status),
                     ...
                     my_draft_prs = ?6,
                     open_issues = COALESCE(?7, open_issues),
                     issues_assigned_to_me = COALESCE(?8, issues_assigned_to_me),
                     deploy_workflow = COALESCE(?9, deploy_workflow),
                     deploy_status = COALESCE(?10, deploy_status),
                     deploy_last_success_ts = COALESCE(?11, deploy_last_success_ts)
                 WHERE id = ?12",
                rusqlite::params![
                    snap.ci,
                    prs.open,
