...
- **MCP App is purely additive.** `repo-recall mcp` exposes the same data layer as the axum dashboard (`db`, `scanner`, `sessions`, `commits`, `activity`, `join`, `routes::refresh`) over MCP stdio. Six tools, one widget. Don't move scan logic into `src/mcp/` — call into existing modules. `recall_refresh` calls `routes::refresh::run_refresh` so axum and MCP modes share the scan implementation, push dispatch, and the periodic refresh loop. The two modes are independent processes; running both side-by-side is fine.
