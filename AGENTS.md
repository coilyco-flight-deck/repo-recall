...
- **No GraphQL.** All GitHub queries route through `gh`'s REST subcommands (`gh pr list`, `gh issue list`, `gh run list`), not `gh api graphql`. GraphQL is harder to debug, harder to rate-limit-reason-about, and the dashboard's needs (counts plus a small handful of fields) fit REST cleanly. If a future feature seems to "need" GraphQL, that's a signal the feature is too rich for this tool, not that the rule should bend. The one historical use - PR file lists piggybacked on the count query - got pulled out when it turned out the file lists weren't worth the GraphQL surface area.
