...
.PHONY: help install run watch build release test fmt fmt-check lint check ci clean smoke
...
run: ## Run the MCP server on stdio against the current directory
	REPO_RECALL_CWD=$(cwd) REPO_RECALL_DEPTH=$(depth) cargo run

watch: ## Rebuild + relaunch the MCP server on every save
	REPO_RECALL_CWD=$(cwd) REPO_RECALL_DEPTH=$(depth) \
...
smoke: build ## Boot the binary, send initialize+tools/list, assert green
...
ci: fmt-check lint check test smoke ## Everything CI runs, in order. Fail fast.
